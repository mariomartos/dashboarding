...
def get_first_block(conn, contract_address, api_key):
    try:
        cursor = conn.cursor()
        # Consulta SQL para obtener el bloque más alto registrado en logs
        query = "SELECT ISNULL(MAX(block_from), 0) FROM logs"
        cursor.execute(query)
        result = cursor.fetchone()
        first_block = result[0] if result else 0
        
        # Si no hay registros en logs, realizar la llamada a la API
        if first_block == 0:
            ...
            try:
                ...
                print(f"Error al obtener el primer bloque desde la API: {e}")
                return None
        ...
        print(f"Error al obtener el primer bloque desde SQL: {e}")
        return None
...
def update_last_refresh(conn, contract_address):
    try:
        ...
        conn.commit()
        print(f"Last refresh actualizado.")
    except Exception as e:
    ...
    if not api_key:
        ...
        if conn:
            ...
            if first_block:
                ...
                # Actualizar last_refresh después de finalizar el procesamiento
